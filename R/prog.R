...
MultiHPD <- function (data, position, level = 0.95)
  {
   result <- matrix(nrow = 0, ncol = 3, dimnames = list(NULL, c("Level","Inferior","Superior")))
   for (i in 1:length(position)) {
      res = cbind(level, matrix(round(hdr(data[, position[i]], prob = c(level * 100))$hdr, digits = 0), ncol = 2, byrow = TRUE))
      rownames(res) <- rep(names(data)[position[i]], times = nrow(res))
...
OccurrencePlot <- function(data, position, level = 0.95, intervals = "CI",
                           ...
                           labelYaxis = NULL,
                           language = "English", occurrence = "event",
...
if (intervals == "CI") {
  Bornes = MultiCredibleInterval(groupOfEvents, 1:ncol(groupOfEvents),
                                 level = level)
}
else if (intervals == "HPD") {
